...
//final lectura y logging ================================================================
   ...
   if( rd.nbt && !rd.nmc ){
    if(llenar_tanque()){
     ...
     if( (rd.ec > or.ec_max) || (rd.ec < or.ec_max) ){
        if(rd.ec > or.ec_max){
          if(llenar_tanque()){
        ...
        if( (rd.pH > or.pH_max) || (rd.pH < or.pH_min) ){
          if(rd.ec > or.ec_max){
            pHmodify(1);
            sleep(180);
          }else{
...
struct c_conf _init_conf( MYSQL* conn, const char * str){
  ...
  /*Realizar un Query a la base de datos para recuperar los 
          rangos optimos de Jitomate*/

 //sprintf(str,"INSERT INTO tLogLector  VALUES (NULL, 1, NOW(), %.3f, NULL, NULL, NULL, NULL, NULL, NULL)", rd.pH);

/*
  if (mysql_query(conn, "SELECT * FROM cHortaliza WHERE nombre ='jitomate'")) 
  {
      db_error(conn);
  }*/
...
/*Comprovas si hay suficiente Agua en Tanque y Cisterna 
y llenar Tanque si se cumple la condicion  ON:  NBT + !NMC
Apagar bomba cuando se cumple la condicion ON: !NAT + NBC
*/
int llenar_tanque(){
  int nat, nbc;
  ...
  if(nat || !nbc){
    ...
    printf("ALARMA!! EC alto no hay agua en la cisterna o el tanque esta lleno\n");
    return -1;
  }
}
...
/*Esta funcion recibe como argumento la dosis
en concentracion ml/lt, la capacidad total del tanque
y la ultima medicion de EC*/
...
//calcudo de dosis  dosis_nominal * ECactual / EC optimo 
  if(rd.ec)
    actualdose = (or.dosis * rd.ec)/mm;
...
// WHAT TO DO=========+++++++++++++++++++=====+++++=======
/*This function has to take as an argument
a string and return as an array of strings*/
...
porque no funciona e DATE_FORMAT esta: 

INSERT INTO tLogLector  VALUES (NULL, 1, DATE_FORMAT(NOW(),'%d/%m/%Y %H:%i:%s'), 3.885, NULL, NULL, NULL, NULL, NULL, NULL)

y en esta si funciona:
